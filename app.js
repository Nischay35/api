...
app.post('/players/', async (request, response) => {
  ...
  const addCricketQuery = `insert into cricket_team (playerName,
        jerseyNumber,
        role) values 
                     ('${playerName}',
                       ${jerseyNumber},
...
app.put('/players/:playerId/', async (request, response) => {
  ...
  const playerDetails = request.body
  const {playerName, jerseyNumber, role} = playerDetails
  const updateCricketQuery = `update cricket_team set playerName='${playerName}', jerseyNumber=${jerseyNumber}, role='${role}' where player_id=${playerId};`
